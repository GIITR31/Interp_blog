...
## **Preliminaries**
### Residual Stream
In a traditional feed-forward network, each layer is a function that transforms the output of the previous layer. If a layer fails to learn something useful, it affects the signal for every subsequent operation. In a Transformer, we use Residual Connections. Every layer(whether it’s an Attention head or an MLP) does not replace the input, but it adds its result to it. The next layer then performs some operations on the result from this addition and then, in turn, adds its output to that input. This is then “read” by the next layer and so on. This creates a communication channel through which the layers “read” the information, perform some operations on it, and then “write” on it (by adding). This channel is called Residual stream.
...
<p align="center">
<img width="600" height="600" alt="clock_animation-3" src="clock_animation-3.gif" />
</p>
